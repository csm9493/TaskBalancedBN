...
def _stack(in_ch: int, out_ch: int, kernel_size: int, stride: int, exp_factor: int, repeats: int,
           bn_momentum: float, norm_layer: nn.Module) -> nn.Sequential:
...
class MNASNet(torch.nn.Module):
    ...
    def __init__(
        ...
        layers = [
            ...
            _stack(depths[1], depths[2], 3, 2, 3, 3, _BN_MOMENTUM, norm_layer),
            _stack(depths[2], depths[3], 5, 2, 3, 3, _BN_MOMENTUM, norm_layer),
            _stack(depths[3], depths[4], 5, 2, 6, 3, _BN_MOMENTUM, norm_layer),
            _stack(depths[4], depths[5], 3, 1, 6, 2, _BN_MOMENTUM, norm_layer),
            _stack(depths[5], depths[6], 5, 2, 6, 4, _BN_MOMENTUM, norm_layer),
            _stack(depths[6], depths[7], 3, 1, 6, 1, _BN_MOMENTUM, norm_layer),
