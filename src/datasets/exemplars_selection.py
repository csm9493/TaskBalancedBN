...
class ExemplarsSelector:
    ...
    def _exemplars_per_class_num(self, model: LLL_Net):
        ...
        if torch.cuda.device_count() > 1:
            num_cls = model.module.task_cls.sum().item()
        else:
...
class RandomExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        if torch.cuda.device_count() > 1:
            num_cls = sum(model.module.task_cls)
        else:
        ...
        #print("exemplars_per_class", exemplars_per_class)
...
#             print ('exemplars_per_class : ' ,exemplars_per_class, 'len(cls_ind) : ',len(cls_ind))
...
class HerdingExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        if torch.cuda.device_count() > 1:
            model_device = next(model.module.parameters()).device  # we assume here that whole model is on a single device
        else:
