...
class BaseDataset_unlabeled(Dataset):
    """Characterizes a dataset for PyTorch -- this dataset pre-loads all paths in memory"""
...
def get_data(path, dataset, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    ...
    root_path = '/YOUR_IMAGENET_DATASET_DIRECTORY/Imagenet'
        
    if 'imagenet100' in dataset:
        train_name = 'imagenet100_train.txt'
        val_name = 'imagenet100_val.txt'
    elif dataset == 'imagenet1000':
        train_name = 'imagenet1000_train.txt'
        val_name = 'imagenet1000_val.txt'
    
    trn_lines = np.loadtxt(os.path.join(path, train_name), dtype=str)
    tst_lines = np.loadtxt(os.path.join(path, val_name), dtype=str)
    ...
    for this_image, this_label in trn_lines:
        if 'JPEG' not in this_image:
            this_image += 'G'
        if not os.path.isabs(this_image):
            this_image = os.path.join(root_path, this_image)
...
def get_data_unsup(path, dataset, num_tasks, nc_first_task, num_classes = 1000):
    """Prepare data: dataset splits, task partition, class order"""

    data = {}
    
    if 'MS_COCO_2017_unsup' in dataset:
#         train_name = 'ms_coco_2017_unsup_train.txt'
        root_path = '/dataset/MS_COCO_2017_unsup'
        trn_lines = [root_path +'/'+ f for f in listdir(root_path) if isfile(join(root_path, f))]
    
#         trn_lines = np.loadtxt(os.path.join(path, train_name), dtype=str)
#     tst_lines = np.loadtxt(os.path.join(path, val_name), dtype=str)

    num_data = len(trn_lines)
    data_order = list(range(len(np.unique(trn_lines))))

    np.random.shuffle(data_order)

    # compute classes per task and num_tasks
    if nc_first_task is None:
        data_size_per_task = np.array([(1 / num_tasks)*num_data] * num_tasks).astype(np.int)
    else:
        data_size_per_task = np.array([(nc_first_task / num_classes)*num_data], [(1-(nc_first_task / num_classes))/(num_tasks-1)] * (num_tasks-1)).astype(np.int)
    
    print ('num_data of ',dataset,' : ', num_data)
    print ('data_size_per_task of ',dataset,' : ', data_size_per_task)
    ...
    # ALL OR TRAIN
    idx = 0
    for t in range(data_size_per_task.shape[0]):
        data_len = data_size_per_task[t]
        
        for j in range(idx, data_len + idx):
            this_image = trn_lines[data_order[j]]
            if not os.path.isabs(this_image):
                this_image = os.path.join(root_path, this_image)
            data[t]['trn']['x'].append(this_image)
